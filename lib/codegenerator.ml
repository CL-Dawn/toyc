...
let initial_context func_name = {
  env = [];
  next_offset = -16;     (* 预留初始栈空间（从-16开始分配局部变量） *)
  label_counter = 0;
...
let callee_saved_regs = ["s0"; "s1"; "s2"; "s3"; "s4"; "s5"; "s6"; "s7"]  (* 被调用者保存寄存器 *)
...
let rec codegen_expr expr ctx =
  ...
  | Call (func, args) ->
        ...
        | arg::rest ->
            ...
            let arg_code, ctx2 = 
              if index < List.length arg_regs then (
                ...
                (* 栈参数：相对sp的偏移（调用者负责分配栈空间） *)
                let stack_pos = (index - List.length arg_regs) * 4 in
                (code @ [ "sw " ^ reg ^ ", " ^ string_of_int stack_pos ^ "(sp)" ], ctx1)
              )
            in
      ...
      (* 准备栈空间（栈参数） *)
      let adjust_stack = 
        if stack_space > 0 then [ "addi sp, sp, -" ^ string_of_int stack_space ]
        else []
      in
      ...
      (adjust_stack @ arg_code @ call_code @ cleanup_stack @ ["mv t0, a0"], "t0", ctx1)
...
let rec codegen_stmt stmt ctx =
  ...
  | Block stmts ->
      (* 保存块开始前的环境和栈偏移（块内变量仅在块内可见） *)
      let saved_env = ctx.env in
      let saved_offset = ctx.next_offset in
...
let codegen_function func =
  ...
  (* 分配参数空间（前8个用寄存器，超过8个用栈） *)
  let (_, ctx_with_params) =
    List.fold_left
      (fun (i, ctx) (typ, name) ->
        let size = size_of typ in
        (* 前8个参数偏移：-16 - i*4（对应a0-a7保存到栈帧） *)
        (* 超过8个的参数偏移：-48 - (i-8)*4（栈上传入的参数） *)
  ...
  (* 计算栈帧大小（16字节对齐，确保容纳所有被保存寄存器和局部变量） *)
  let frame_size = 
    let min_offset = ctx_body.next_offset in
    let total = (-min_offset + 15) land (lnot 15) in  (* 16字节对齐 *)
    (* 最小栈帧大小：ra(4字节) + 所有被调用者保存寄存器(8*4=32字节) = 36字节 *)
    max total (4 + List.length callee_saved_regs * 4)
  in
  
  (* 保存被调用者寄存器（s0-s7）到栈帧，按顺序保存 *)
  let save_callee_regs = 
    List.mapi (fun i reg ->
      let offset = frame_size - 4 - (i + 1) * 4 in  (* ra在frame_size-4，s0在frame_size-8，依次递减 *)
      "sw " ^ reg ^ ", " ^ string_of_int offset ^ "(sp)")
    callee_saved_regs
  in
  
  (* 恢复被调用者寄存器（逆序恢复，与保存顺序相反） *)
  let restore_callee_regs = 
    List.mapi (fun i reg ->
      let offset = frame_size - 4 - (i + 1) * 4 in  (* 与保存时的偏移一致 *)
      "lw " ^ reg ^ ", " ^ string_of_int offset ^ "(sp)")
    (List.rev callee_saved_regs)  (* 逆序恢复（s7先恢复，最后恢复s0） *)
  in
  
  (* 函数序言（保存ra、被调用者寄存器，设置栈帧基址） *)
  let prologue = [
    func.name ^ ":";
    "addi sp, sp, -" ^ string_of_int frame_size;  (* 分配栈帧空间 *)
    "sw ra, " ^ string_of_int (frame_size - 4) ^ "(sp)";  (* 保存返回地址ra *)
  ] @ save_callee_regs @ [
    "addi s0, sp, " ^ string_of_int frame_size;  (* 设置栈帧基址s0 = sp + frame_size *)
  ] @ param_save_code  (* 保存参数寄存器到栈帧 *)
  in
  
  (* 函数结语（恢复寄存器，释放栈帧，返回） *)
  let epilogue = [
    func.name ^ "_exit:";  (* 函数退出标签 *)
  ] @ restore_callee_regs @ [  (* 恢复被调用者寄存器 *)
    "lw ra, " ^ string_of_int (frame_size - 4) ^ "(sp)";  (* 恢复返回地址ra *)
    "addi sp, sp, " ^ string_of_int frame_size;  (* 释放栈帧空间 *)
    "ret"  (* 返回 *)
  ] in
...
let codegen_program (program : program) =
  ...
  (* 程序退出指令（仅在存在main函数时添加，且不重复定义main_exit） *)
  let exit_code = 
    if List.exists (fun f -> f.name = "main") program then
      ["li a7, 93"; "ecall"]  (* 系统调用：exit() *)
    else [] 
  in
